...
export function riskForAnswers(answers: ProjectAnswers): { risk: RiskLevel; reasons: string[] } {
  ...
  if (answers.features.includes("login")) {
    reasons.push("包含登录，需要会话、账号恢复和权限边界设计。");
    return { risk: "high", reasons };
  }

  if (["app", "ecommerce", "internal"].includes(answers.websiteType)) {
    reasons.push("网站包含持续性业务状态或内部流程，不能只按静态页面验收。");
    return { risk: "medium", reasons };
  }

  reasons.push("当前选择以公开内容为主，未声明登录、支付、上传或个人信息处理。");
  return { risk: "low", reasons };
}

function baseRoutes(answers: ProjectAnswers): {
  primary: RouteOption;
  simple: RouteOption;
...
export function recommendationFor(answers: ProjectAnswers): RouteRecommendation {
  const riskResult = riskForAnswers(answers);
  const routes = baseRoutes(answers);
  ...
  if (["global", "both"].includes(answers.region)) {
    constraints.push("全球访问需验证 CDN、语言、时区、隐私与目标市场的支付/数据要求。");
  }
  if (["operator", "approval"].includes(answers.contentOwner)) {
    constraints.push("内容由运营或多人维护：技术决策必须包含 CMS、预览、权限、审批与回滚内容版本的方案。");
  }
  if (answers.priority === "ownership") {
    constraints.push("代码所有权优先：在 ADR 中记录导出能力、数据格式、域名控制和供应商迁移路径。");
  }
  if (answers.priority === "cost") {
    constraints.push("成本优先：同时记录人力维护、插件/应用、流量增长和迁移成本，不能只看首月价格。");
  }
...
export function currentStage(project: WebsiteProject): StageDefinition {
  for (const stage of STAGES) {
    ...
    if (progress?.status !== "done" || !stageCanBeDone(project, stage)) return stage;
  }
  return STAGES[STAGES.length - 1];
}

export function nextAction(project: WebsiteProject): {
  ...
  if (stageProgress?.status === "blocked" && stageProgress.blocker.trim()) {
    return {
      stage,
      title: `解除阻塞：${stageProgress.blocker.trim()}`,
      detail: "先明确负责人、缺失资料或需要的新授权；不要让 AI 猜测后继续。",
    };
  }
  ...
  if (stageProgress?.status !== "done") {
    return {
      stage,
      title: `把“${stage.title}”提交验收`,
      detail: "适用 Gate 已有证据；请由相应负责人复核后再标记完成。",
    };
  }
...
export function createProject(
  ...
  return {
    schemaVersion: 2,
    id,
    name: name.trim() || "未命名网站计划",
    createdAt: timestamp,
    updatedAt: timestamp,
    answers,
    recommendation: recommendationFor(answers),
    stages,
    promptDrafts: {},
  };
}

export function projectSummary(project: WebsiteProject): string {
  const type = labelFor(WEBSITE_TYPES, project.answers.websiteType);
  const region = labelFor(REGIONS, project.answers.region);
  const owner = labelFor(CONTENT_OWNERS, project.answers.contentOwner);
  const priority = labelFor(PRIORITIES, project.answers.priority);
  const features = project.answers.features.length
    ? project.answers.features.map((item) => labelFor(FEATURES, item)).join("、")
    : "无登录、权限、个人信息、上传或支付";
  return `${type}；主要访问地域：${region}；内容维护：${owner}；优先级：${priority}；声明功能：${features}。`;
}

export function promptDefaults(project: WebsiteProject, template: PromptTemplate): Record<string, string> {
  const values: Record<string, string> = {};
  for (const field of template.fields) values[field.id] = "";
  if ("projectName" in values) values.projectName = project.name;
  if ("regionConstraints" in values) values.regionConstraints = labelFor(REGIONS, project.answers.region);
  if ("teamConstraints" in values) {
    values.teamConstraints = `内容维护者：${labelFor(CONTENT_OWNERS, project.answers.contentOwner)}；优先级：${labelFor(PRIORITIES, project.answers.priority)}。`;
  }
  if ("requirements" in values) values.requirements = projectSummary(project);
  return values;
}
...
export function composePrompt(
  ...
  const fieldLines = template.fields.map((field) => {
    ...
    if (unknowns.includes(field.id)) return `- ${field.label}：未知；请先追问，不得猜测。`;
    return `- ${field.label}：${value || "未提供"}`;
  });
  ...
  return [
    ...
    "",
    "## 技术与业务约束",
    ...project.recommendation.constraints.map((item) => `- ${item}`),
...
export function validateImportedProject(value: unknown): WebsiteProject {
  if (!value || typeof value !== "object") throw new Error("文件不是项目对象。");
  const candidate = value as Partial<WebsiteProject>;
  if (candidate.schemaVersion !== 2) throw new Error("项目版本不受支持，需要 schemaVersion 2。");
  if (typeof candidate.id !== "string" || typeof candidate.name !== "string") {
    throw new Error("项目缺少 id 或名称。");
  }
  if (!candidate.answers || !WEBSITE_TYPES.some((item) => item.id === candidate.answers?.websiteType)) {
    throw new Error("项目的网站类型无效。");
  }
  if (!candidate.stages || typeof candidate.stages !== "object") throw new Error("项目缺少阶段数据。");
  if (!candidate.recommendation) candidate.recommendation = recommendationFor(candidate.answers);
...
export function promptForStage(stageId: number): PromptTemplate {
  const stage = STAGES.find((item) => item.id === stageId) ?? STAGES[0];
  return PROMPT_TEMPLATES.find((template) => stage.promptIds.includes(template.id)) ?? PROMPT_TEMPLATES[0];
}
